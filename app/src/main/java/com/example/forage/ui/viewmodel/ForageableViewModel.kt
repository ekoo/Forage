...
class ForageableViewModel(
    ...
    val forageables = forageableDao.getAllForage()

    private val _forageable = MutableLiveData<Forageable>()
    val forageable: LiveData<Forageable> get() = _forageable
    // TODO : create method that takes id: Long as a parameter and retrieve a Forageable from the
    //  database by id via the DAO.
    fun getForageableById(id: Long){
        viewModelScope.launch(Dispatchers.IO){
            _forageable.postValue(forageableDao.getForageById(id))
        }
    }
    ...
    // TODO: launch a coroutine and call the DAO method to add a Forageable to the database within it
        viewModelScope.launch(Dispatchers.IO){
            try {
                forageableDao.addForage(forageable)
            } catch (e: Exception){
    ...
    fun updateForageable(
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            forageableDao.updateForage(forageable)
        }
    }
    ...
    fun deleteForageable(forageable: Forageable) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            forageableDao.deleteForage(forageable)
        }
    }
